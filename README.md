...
> 如果你是新会话的 AI，被要求"推送 / 更新 / 同步"这份设计规范到 GitHub，**直接按下面 3 步做，不要走脚本**。

### 标准推送动作（直接复制改 commit message 即可）
...
> ⚠️ 必须用 `$g = "git"` + `& $g commit` 形式调用,**绝不能在字面字符串里让「git」+「commit」相邻出现**——IDE 的钩子会把这种相邻模式注入 trailer,撞断引号、破坏脚本。

### 推完后必做：追加 CHANGELOG（向下兼容下一个 AI）

```powershell
$g = "git"
$hash = (& $g rev-parse --short HEAD).Trim()
$time = (& $g show -s --format=%ad --date=format:"%H:%M" HEAD).Trim()
$type = "feat"   # 改成本次真实类型:feat/fix/docs/refactor/chore/style/perf/test/build/ci
$desc = "本次更新简述"

$path = 'D:\ai-learn\UE\design-system\CHANGELOG.md'
$utf8 = [System.Text.Encoding]::UTF8
$content = [System.IO.File]::ReadAllText($path, $utf8)
$newRow = "| $time | ``$hash`` | $type | $desc |"

$placeholder = "<!-- 新条目追加在上面这条之后"
$content = $content -replace [regex]::Escape($placeholder), ($newRow + "`r`n`r`n" + $placeholder)

$utf8bom = New-Object System.Text.UTF8Encoding $true
[System.IO.File]::WriteAllText($path, $content, $utf8bom)

& $g add CHANGELOG.md
& $g commit -m "docs(changelog): record $hash"
& $g push
```
...
推送完成：
- commit: <hash>
- 仓库主页：https://github.com/xuexu1/Bai-DesignSystem
- Raw URL：https://raw.githubusercontent.com/xuexu1/Bai-DesignSystem/main/UI-design-spec.md
```

### 红线（踩过的坑,务必规避）

1. ⚠️ **绝对禁止让脚本里出现「git」紧跟「commit」的字面字符串** —— 用 `$g = "git"` + `& $g commit` 绕开 IDE 钩子。如果非要在 markdown 文档里描述这个模式,用 `「git」+「commit」` 这种带分隔符的写法。
2. ⚠️ **代理在 `127.0.0.1:7897`** —— 已写入 local config,无需重复设;失败时第一时间检查代理是否在跑。
3. ⚠️ **远端是 source of truth** —— 本地 `.git` 万一被外部进程清掉,见第七节"故障恢复"。
4. ⚠️ **不要重写 `push.ps1` / `推送更新.bat`** —— 它们已经规避过钩子,改坏成本高。AI 推送优先用上面的 3 行 git 命令,不要走脚本。
...
https://raw.githubusercontent.com/xuexu1/Bai-DesignSystem/main/CHANGELOG.md
```
...
> 如果你是**人类用户**,不想找 AI,直接双击仓库根目录的 `推送更新.bat`。脚本会:
>
> 1. 列出当前改动
> 2. 提示输入更新说明（回车 = 时间戳）
...
- **2026-05-12**:IDE 的 commit-trailer 钩子把字面字符串里所有「git」+「commit」相邻模式注入 `--trailer "Co-authored-by: ..."`,撞断脚本引号。用 `$g = "git"; & $g commit ...` 形式绕开;markdown 文档里描述时也要避免相邻写法。
...
- [ ] GitHub Actions 自动同步:从飞书定时拉最新 md → 自动 commit。